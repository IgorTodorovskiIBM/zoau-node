...
class Dataset {
  ...
  async read(args={}) {
    return await read(this.get_name(), args);
  }

  async write(content, append=false, args={}) {
    return await write(this.get_name(), content, append, args);
  }

  async delete(args={}) {
    return await delete(this.get_name());
  }
}
...
async function create(name, type="pds", args={}) {
  ...
  var dslist = await listing(name, {"detailed" : true});
  return dslist[0];
}
...
async function find_member(member, concatentation, args={}) {
  ...
  // TODO(gabylb): dwhence returns either 0 (found) or 8 (not found or invalid dataset, etc)
...
async function _lineinfile(dataset, line, regex=null, args={}) {
  ...
  if ("state" in args) {
    if (typeof args["state"] !== "boolean") {
      response = {rc: 55, stdout: "", command: "_lineinfile()", stderr: "state should be true or false"};
      throw new Error(JSON.stringify(response));
    } else {
      state = args["state"];
    }
  ...
  if ("lock" in args) {
    if (typeof args["lock"] !== "boolean") {
      response = {rc: 55, stdout: "", command: "_lineinfile()", stderr: "lock should be true or false"};
      throw new Error(JSON.stringify(response));
    }
    if (args["lock"])
      options += " -l";
  ...
  if ("backref" in args) {
    if (typeof args["backref"] !== "boolean") {
      response = {rc: 55, stdout: "", command: "_lineinfile()", stderr: "backref should be true or false"};
      throw new Error(JSON.stringify(response));
    }
    if (args["backref"])
      options += " -r";
  ...
  if (state) { 
    if (regex) {
      ...
      if (ins_aft) {
        ...
        response = {rc: 55, stdout: "", command: "_lineinfile()", stderr: "Incorrect parameters"};
        throw new Error(JSON.stringify(response));
      }
    }
  } else {
...
async function _blockinfile(dataset, block=null, args={}) {
  ...
  if ("state" in args) {
    if (typeof args["state"] !== "boolean") {
      response = {rc: 55, stdout: "", command: "_blockinfile()", stderr: "state should be true or false"};
      throw new Error(JSON.stringify(response));
    } else
      state = args["state"];
  }
  if ("lock" in args) {
    if (typeof args["lock"] !== "boolean") {
      response = {rc: 55, stdout: "", command: "_blockinfile()", stderr: "lock should be true or false"};
      throw new Error(JSON.stringify(response));
    } else if (args["lock"])
      options += " -l";
  }
  ...
  if (state) { 
    if (!block) {
      response = {rc: 55, stdout: "", command: "_blockinfile()", stderr: "block is required when state=true"};
      throw new Error(JSON.stringify(response));
    } else if (ins_aft) {
      if (ins_aft === "EOF")
      ...
      response = {rc: 55, stdout: "", command: "_blockinfile()", stderr: "insertafter or insertbefore is required when state=true"};
      throw new Error(JSON.stringify(response));
    }
  } else {
    options += ` \"//d\" \"${dataset}\"`;

...
if (process.platform !== 'os390') {
  console.log(`node-zoau ERR! this module is for use on z/OS only: wanted: ` +
...
execFile('zoaversion', (error, stdout, stderr) => {
  if (error || stderr || !stdout) {
    if (String(stderr).match(/CEE3201S.+call_shared_function.+/s)) {
      console.log(`node-zoau ERR! ZOA Utilities is not correctly installed ` +
                  `on your system (please ensure you have added ZOA ` +
                  `Utilities to the LIBPATH environment variable)`);
    } else {
      console.log(`node-zoau ERR! ZOA Utilities is not installed on your ` +
  ...
  if (major < 1 ||
    ...
    console.log(`node-zoau ERR! version of ZOA Utilities installed on your ` +
...
execFile('mvscmdauth', (error, stdout, stderr) => {
  ...
  if (stdout || !stderr) {
    console.log(`node-zoau ERR! ZOA Utilities is not correctly installed on ` +
                `your system`);
    process.exit(1);
  }
